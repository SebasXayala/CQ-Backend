...
export class FolderService {
  ...
  async findByCandidate(candidateId: number): Promise<Folder[]> {
    ...
    try {
      ...
      throw new BadRequestException(`Error al buscar carpetas para el candidato`);
    }
  }
  ...
  async remove(id: number): Promise<Folder> {
    ...
    try {
      ...
      if (error.code === '23503') {
        throw new ConflictException(`No se puede eliminar la carpeta porque tiene archivos almacenados`);
      }
      throw new BadRequestException(`Error al eliminar carpeta`);
    }
  }
}
